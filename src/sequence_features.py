...
# Module-level caches for fast repeated access
_pom_rank_cache: dict[int, dict[int, int]] = {}
_season_detail_cache: dict[int, pd.DataFrame] = {}


def _get_pom_ranks(massey_df, season, day_cutoff=133):
    """Get latest POM ranks for a season (cached)."""
    if season in _pom_rank_cache:
        return _pom_rank_cache[season]
    df = massey_df[
    ...
    if df.empty:
        _pom_rank_cache[season] = {}
        return {}
    latest_day = df["RankingDayNum"].max()
    df = df[df["RankingDayNum"] == latest_day]
    result = dict(zip(df["TeamID"], df["OrdinalRank"]))
    _pom_rank_cache[season] = result
    return result


def _get_season_detail(data, season):
    """Get detailed results for a season (cached)."""
    if season in _season_detail_cache:
        return _season_detail_cache[season]
    detail = data.get("regular_detail")
    if detail is None:
        return pd.DataFrame()
    df = detail[detail["Season"] == season]
    _season_detail_cache[season] = df
    return df
...
def build_team_season_sequence(data, team_id, season, max_games=35):
    ...
    """
    sg = _get_season_detail(data, season)
    if sg.empty:
    ...
    # Get all games for this team in this season (from cached season data)
    won = sg[sg["WTeamID"] == team_id]
    lost = sg[sg["LTeamID"] == team_id]
...
def build_team_partial_sequence(data, team_id, season, max_day, max_games=35):
    """
    Build game embedding sequence truncated to games before max_day.
    Used for pretraining: ensures no future information leaks.

    Returns:
        np.ndarray of shape (max_games, 13), padded/truncated
        int: actual number of games
    """
    detail = data.get("regular_detail")
    if detail is None:
        return np.zeros((max_games, 13)), 0

    massey = data.get("massey")
    pom_ranks = _get_pom_ranks(massey, season) if massey is not None else {}
    max_rank = 363

    won = detail[(detail["Season"] == season) & (detail["WTeamID"] == team_id) & (detail["DayNum"] < max_day)].copy()
    lost = detail[(detail["Season"] == season) & (detail["LTeamID"] == team_id) & (detail["DayNum"] < max_day)].copy()
    ...
    feature_keys = [
        ...
        "fg_pct", "fg3_pct", "ft_pct", "reb_rate",
        "ast", "to", "stl", "blk",
        "win", "day",  # use raw day as last feature (normalized below)
    ]

    n_games = len(games)
    min_day = games[0]["day"]
    max_day_val = games[-1]["day"]
    day_range = max(max_day_val - min_day, 1)

    arr = np.zeros((max_games, 13))
    for i, g in enumerate(games[:max_games]):
        vals = [g[k] for k in feature_keys[:-1]]  # first 12
        vals.append((g["day"] - min_day) / day_range)  # day_norm as 13th
        arr[i] = vals
